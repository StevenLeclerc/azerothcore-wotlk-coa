...
def main():
    ...
    native = load("falcon_compile", require_helper(args.workspace_tools / "Test-LocalLoginCollections.py"))
    base = load("flower_fixture", Path(__file__).parent.parent / "ranger_flowers/run.py")
    code = base.make_harness(native, ROOT / "modules/mod-ascension-compat/src/AscensionRangerFalconstrike.cpp", "ScriptedAI")
    ...
    code += "\n" + (ROOT / "modules/mod-ascension-compat/src/AscensionCustomResourceData.h").read_text()
    resource = (ROOT / "modules/mod-ascension-compat/src/AscensionCompat.cpp").read_text()
